...
def on_tab_click(sectionframes, sectionbuttons, currentFrame, currentbutton):
    print("errorD", currentFrame)
    print("errorT", type(currentFrame))
    ...
    if currentFrame != sectionframes[5]:
        ...
        if currentFrame == sectionframes[0]:
            ...
            generate_Text_Button = CTkButton(currentFrame, text=" Generate", font=("Poppins", 20), border_width=0, corner_radius=32,
                                             fg_color="#00BB6D", width=600, height=50, hover_color="null", command=lambda: generate_text(currentFrame, result_label, uploaded_image, resultText))
...
# Fucntion for document scanner frame to only create QR code to redirect the mobile app.
...
def generate_text(currentFrame, result_label, uploaded_image, resultText):
    ...
    if uploaded_image is None:
        ...
        extractedText = pytesseract.image_to_string(gray)
...
def download_image(currentFrame, uploaded_image):
    ...
    if uploaded_image is None:
        ...
        closeButton = CTkButton(t, text="Ok", fg_color="transparent", hover_color="null",
